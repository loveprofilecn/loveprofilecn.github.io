...
test("scores are deterministic and avoid misleading absolutes",()=>{const answers=pick("max"),a=calculateDimensionScores(answers),b=calculateDimensionScores(answers);assert.deepEqual(a,b);for(const v of Object.values(a))assert.ok(v>=6&&v<=94)});
